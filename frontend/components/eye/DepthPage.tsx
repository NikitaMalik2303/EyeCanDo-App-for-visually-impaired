...
export default function DepthPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      socket.current.on("depth_detection_result", (data) => {
        console.log("Object detection result:", data);
        console.log(`Received at ${Date.now()}`);
        if (data) {
          console.log(data);
          const message = `Obstacle detected on ${data["region"]}`;
  ...
  const streamFrames = async () => {
    ...
    try {
      socket.current?.emit("depth_detection_frame", photo.base64);
    } catch (error) {
        ...
        >
          <BottomSheetViewComponent>
            <BottomSheetHeaderContainer>
              ...
              <Heading>Objects detected</Heading>
            </BottomSheetHeaderContainer>
            <ResultText>
              {/* {apiResult?.[0]?.name || "Unknown"} from your contacts is in front
              of you. Greet them! */}
              {apiResult}
            </ResultText>
          </BottomSheetViewComponent>
